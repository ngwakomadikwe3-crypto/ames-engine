...
function JewelryModel({
  ...
  useEffect(() => {
    ...
    new GLTFLoader().load(
      ...
      undefined,
      (error) => console.error(`AMES Engine Lab failed to load ${url}`, error),
    )
      ...
      <div style={{ position: 'absolute', zIndex: 10, top: 18, left: 20 }}>
        ...
        <div style={{ marginTop: 12, display: 'flex', gap: 8 }}>
          {MODELS.map((model) => {
            ...
            return (
              <button key={model.url} onClick={() => changeModel(model.url)} style={{ border: active ? '1px solid #d6b76b' : '1px solid #333', borderRadius: 8, padding: '8px 11px', background: active ? '#201c12' : '#151515', color: '#fff', cursor: 'pointer', fontSize: 11, fontWeight: active ? 700 : 500 }}>
                {model.label}
              </button>
            )
          })}
        ...
        <aside style={{ position: 'absolute', zIndex: 20, top: 18, right: 20, width: 330, maxHeight: 'calc(100vh - 36px)', overflowY: 'auto', padding: 16, border: '1px solid #2b2b2b', borderRadius: 12, background: 'rgba(14,14,14,0.92)', backdropFilter: 'blur(12px)' }}>
          ...
          <div style={{ marginTop: 10, display: 'grid', gap: 8 }}>
            {analysis.parts.map((part) => {
              ...
              return (
                <button key={part.meshId} onClick={() => setSelectedMeshId((current) => current === part.meshId ? undefined : part.meshId)} style={{ textAlign: 'left', border: selectedMeshId === part.meshId ? '1px solid #d6b76b' : '1px solid #2d2d2d', borderRadius: 9, padding: '10px 11px', background: selectedMeshId === part.meshId ? '#201c12' : '#151515', color: '#fff', cursor: 'pointer' }}>
                  ...
                  <div style={{ marginTop: 5, paddingTop: 5, borderTop: '1px solid #292929', fontSize: 10 }}>
                    <span style={{ opacity: 0.45 }}>RENDER ROLE </span>
                    <strong style={{ color: '#eee' }}>{assignment?.role ?? 'PRESERVE_SOURCE'}</strong>
                  </div>
                  {assignment?.sourceMaterialNames.length ? (
                    <div style={{ marginTop: 3, fontSize: 9, opacity: 0.45 }}>Source: {assignment.sourceMaterialNames.join(', ')}</div>
                  ) : null}
                </button>
              )
            })}
