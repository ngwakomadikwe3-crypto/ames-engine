...
function IsolatedDiamond({ onReady }: { onReady: (name: string) => void }) {
  ...
  const diamondMaterial = useMemo(() => createDiamondCoreMaterial(), [])
  ...
  useEffect(() => {
    ...
    }, undefined, error => console.error('AMES Diamond Core failed to load', error))
  ...
  return <>
    ...
    <mesh
      ...
      quaternion={stone.quaternion}
      scale={stone.scale}
    />
    <DiamondCorePathTracer enabled />
  </>
}
...
export function EngineLabPage() {
  ...
  return <main style={{ width:'100vw', height:'100vh', background:'#0b0b0d', color:'#fff', fontFamily:'Inter,system-ui,sans-serif' }}>
    <div style={{ position:'absolute', zIndex:10, top:20, left:22 }}>
      <strong style={{ fontSize:13, letterSpacing:'.18em' }}>AMES DIAMOND CORE</strong>
      <span style={{ fontSize:9, opacity:.5, marginLeft:8 }}>v0.1</span>
      <div style={{ marginTop:9, fontSize:13, opacity:.78 }}>Center Diamond — GPU path-traced benchmark</div>
      <div style={{ marginTop:5, fontSize:10, color:'#9fb5cf' }}>Real geometry · physical transmission · multi-bounce transport</div>
    </div>
    ...
    <aside style={{ position:'absolute', zIndex:20, top:20, right:20, width:300, padding:16, border:'1px solid #2b2b2f', borderRadius:12, background:'rgba(13,13,15,.95)' }}>
      <div style={{ fontSize:11, letterSpacing:'.16em', fontWeight:700, opacity:.68 }}>DIAMOND CORE CHECKPOINT</div>
      <div style={{ marginTop:7, fontSize:9, color:'#d6b76b' }}>GPU PATH TRACING</div>
      ...
      <div style={{ marginTop:10, padding:11, border:'1px solid #303035', borderRadius:8, background:'#171719', fontSize:10, lineHeight:1.6 }}>
        Renderer: <strong>WebGLPathTracer</strong><br/>
        IOR: <strong>2.417</strong><br/>
        Transmission: <strong>1.0</strong><br/>
        Roughness: <strong>0.0</strong><br/>
        Light bounces: <strong>12</strong>
      </div>
      <div style={{ marginTop:11, fontSize:9, lineHeight:1.55, opacity:.5 }}>Pass/fail benchmark. We reconnect the 171-part ring only after this isolated stone reads convincingly as diamond.</div>
    </aside>

    <Canvas camera={{ fov:36, position:[3,2,4] }} dpr={[1,1.25]} gl={{ antialias:true, toneMapping:ACESFilmicToneMapping, toneMappingExposure:1.05 }}>
